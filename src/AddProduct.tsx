...
function AddProduct({ onProductCreated }: AddProductProps) {
  ...
  const createProduct = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const product = await window.adminProducts.createProduct(request)
  ...
  return (
    <section className="product-panel" aria-labelledby="add-product-title">
      ...
      <form className="product-form" onSubmit={(event) => void createProduct(event)}>
        ...
        <label>Colorful Life Category<select aria-label="Colorful Life Category" value={colorfulLifeCategory} onChange={(event) => setColorfulLifeCategory(event.target.value as ColorfulLifeCategory)} required disabled={!!createdProduct}><option value="">Select a category</option>{colorfulLifeCategoryOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        ...
        <label>Condition<select value={condition} onChange={(event) => setCondition(event.target.value as CreateProductRequest['condition'])} disabled={!!createdProduct}><option value="NEW">New</option><option value="USED_LIKE_NEW">Used like new</option></select></label>
        ...
        <label>Current stock<input type="number" min="0" step="1" value={currentStock} onChange={(event) => setCurrentStock(event.target.value)} disabled={!!createdProduct} /></label>
